...
REM Usage: Call "$(MSBuildProjectDirectory)\App_Data\PostBuild.$(Configuration).bat" "$(MSBuildProjectDirectory)\$(OutDir)" "$(Configuration)" "$(ProjectName)" "$(RootNameSpace)"
...
%WINDIR%\system32\xcopy.exe %FullPath%Properties\*.rd.xml %LibFolder%\%RootNamespace%\Properties\*.* /s/r/y
