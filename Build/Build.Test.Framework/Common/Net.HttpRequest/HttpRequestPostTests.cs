...
namespace Genesys.Build.Test
{
    ...
    public class HttpRequestPostTests
    {
        ...
        public async Task Net_HttpRequestPostString_SendAsync()
        {
            ...
            var request = new HttpRequestPostString(configuration.AppSettingValue(AppSettingList.MyWebServiceKeyName) + "/HomeApi");
        ...
        public async Task Net_HttpRequestPost_SendAsync()
        {
            object dataOut;
            var configuration = ConfigurationManagerSafeTests.ConfigurationManagerSafeConstruct();
            var request = new HttpRequestPost<object>(configuration.AppSettingValue(AppSettingList.MyWebServiceKeyName) + "/HomeApi");
            dataOut = await request.SendAsync();
            Assert.IsTrue(request.Response.IsSuccessStatusCode == true);
        }
    }
