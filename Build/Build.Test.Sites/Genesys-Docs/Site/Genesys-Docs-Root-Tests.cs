//-----------------------------------------------------------------------
// <copyright file="Genesys_Docs_Root_Tests.cs" company="Genesys Source">
...
{
    [TestClass()]
    public class Genesys_Docs_Root_Tests
    {
        [TestMethod()]
        public async Task Genesys_Docs_Root()
        {
            var configuration = ConfigurationManagerSafeTests.ConfigurationManagerSafeConstruct();
            var dataOut = TypeExtension.DefaultString;            
            var request = new HttpRequestGetString(configuration.AppSettingValue("GenesysDocsRoot"));
