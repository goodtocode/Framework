//-----------------------------------------------------------------------
// <copyright file="GenesysSource_Tests.cs" company="Genesys Source">
...
{
    [TestClass()]
    public class GenesysSource_Tests
    {
        [TestMethod()]
        public async Task GenesysSource_Root()
        {
            var configuration = ConfigurationManagerSafeTests.ConfigurationManagerSafeConstruct();
            var dataOut = TypeExtension.DefaultString;            
            var request = new HttpRequestGetString(configuration.AppSettingValue("GenesysSourceRoot"));
